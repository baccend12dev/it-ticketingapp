...
<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
        <span class="h2 mb-0 text-dark">Category List</span>
        <button class="btn btn-primary btn-sm py-1 d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#addCatModal">
            ...
            <table class="table table-hover align-middle mb-0" style="font-size: 14px;">
                ...
                <tbody>
                    @forelse($categories as $cat)
                        <tr>
                            <td class="ps-3 text-secondary fw-semibold">#{{ $cat->id }}</td>
                            <td><span class="fw-semibold text-dark">{{ $cat->name }}</span></td>
                            <td class="text-secondary text-truncate" style="max-width: 300px;">{{ $cat->description ?? '-' }}</td>
                            <td>
                                <span class="badge-pill-custom @if($cat->is_active) badge-active @else badge-critical @endif">
                            ...
                            <td class="pe-3 text-end">
                                ...
                                <form action="{{ route('categories.destroy', $cat) }}" method="POST" class="d-inline" 
                                      onsubmit="return confirm('Are you sure you want to delete category \'{{ $cat->name }}\'?');">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <div class="modal-content" style="border-radius: var(--rounded-default); border: 0; box-shadow: var(--shadow-lg);">
                                    ...
                                    <form action="{{ route('categories.update', $cat) }}" method="POST">
                                        ...
                                        <div class="modal-body text-start">
                                            <div class="mb-3">
                                                <label for="edit_name_{{ $cat->id }}" class="form-label label text-secondary">CATEGORY NAME</label>
                                                <input type="text" name="name" id="edit_name_{{ $cat->id }}" class="form-control" value="{{ $cat->name }}" required>
                                            </div>
                                            
                                            <div class="mb-3">
                                                <label for="edit_desc_{{ $cat->id }}" class="form-label label text-secondary">DESCRIPTION</label>
                                                <textarea name="description" id="edit_desc_{{ $cat->id }}" class="form-control" rows="3">{{ $cat->description }}</textarea>
                                            </div>

                                            <div class="mb-3 form-check form-switch d-flex align-items-center gap-2">
                                                <input class="form-check-input mt-0" type="checkbox" name="is_active" value="1" id="edit_active_{{ $cat->id }}" {{ $cat->is_active ? 'checked' : '' }}>
                                                <label class="form-check-label label text-secondary" style="margin-top: 2px;" for="edit_active_{{ $cat->id }}">IS ACTIVE</label>
                                            </div>
                                        </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-4 text-secondary">No categories found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="addCatModal" tabindex="-1" aria-labelledby="addCatModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: var(--rounded-default); border: 0; box-shadow: var(--shadow-lg);">
            <div class="modal-header border-bottom">
                <h5 class="modal-title fw-bold text-dark" id="addCatModalLabel">
            ...
            <form action="{{ route('categories.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="name" class="form-label label text-secondary">CATEGORY NAME</label>
                        <input type="text" name="name" id="name" class="form-control" placeholder="e.g. Network & Connectivity" required>
                    </div>
                    
                    <div class="mb-3">
                        <label for="description" class="form-label label text-secondary">DESCRIPTION</label>
                        <textarea name="description" id="description" class="form-control" rows="3" placeholder="Brief details about the category..."></textarea>
                    </div>
                ...
                <div class="modal-footer border-top">
                    <button type="button" class="btn btn-ghost" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-primary">
                        Create Category <i class="bi bi-check-lg ms-1"></i>
                    </button>
                </div>
            </form>
        </div>
