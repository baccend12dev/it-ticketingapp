...
<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between">
        <span class="h2 mb-0 text-dark">Department List</span>
        <button class="btn btn-primary btn-sm py-1 d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#addDeptModal">
            ...
            <table class="table table-hover align-middle mb-0" style="font-size: 14px;">
                ...
                <tbody>
                    @forelse($departments as $dept)
                        <tr>
                            <td class="ps-3 text-secondary fw-semibold">#{{ $dept->id }}</td>
                            <td><span class="fw-semibold text-dark">{{ $dept->name }}</span></td>
                            <td class="text-secondary text-truncate" style="max-width: 300px;">{{ $dept->description ?? '-' }}</td>
                            <td>
                                <span class="badge-pill-custom @if($dept->is_active) badge-active @else badge-critical @endif">
                            ...
                            <td class="pe-3 text-end">
                                ...
                                <button class="btn btn-outline-dark btn-sm px-2 py-1 me-1" 
                                        ...
                                        data-bs-toggle="modal" data-bs-target="#editDeptModal-{{ $dept->id }}">
                                ...
                                <div class="modal-content" style="border-radius: var(--rounded-default); border: 0; box-shadow: var(--shadow-lg);">
                                    <div class="modal-header border-bottom">
                                        <h5 class="modal-title fw-bold text-dark">
                                            <i class="bi bi-pencil me-2 text-primary"></i>Edit Department
                                        </h5>
                                    ...
                                    <form action="{{ route('departments.update', $dept) }}" method="POST">
                                        ...
                                        <div class="modal-body text-start">
                                            <div class="mb-3">
                                                <label for="edit_name_{{ $dept->id }}" class="form-label label text-secondary">DEPARTMENT NAME</label>
                                                <input type="text" name="name" id="edit_name_{{ $dept->id }}" class="form-control" value="{{ $dept->name }}" required>
                                            </div>
                                            
                                            <div class="mb-3">
                                                <label for="edit_desc_{{ $dept->id }}" class="form-label label text-secondary">DESCRIPTION</label>
                                                <textarea name="description" id="edit_desc_{{ $dept->id }}" class="form-control" rows="3">{{ $dept->description }}</textarea>
                                            </div>

                                            <div class="mb-3 form-check form-switch d-flex align-items-center gap-2">
                                                <input class="form-check-input mt-0" type="checkbox" name="is_active" value="1" id="edit_active_{{ $dept->id }}" {{ $dept->is_active ? 'checked' : '' }}>
                                                <label class="form-check-label label text-secondary" style="margin-top: 2px;" for="edit_active_{{ $dept->id }}">IS ACTIVE</label>
                                            </div>
                                        </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-4 text-secondary">No departments found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="addDeptModal" tabindex="-1" aria-labelledby="addDeptModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: var(--rounded-default); border: 0; box-shadow: var(--shadow-lg);">
            <div class="modal-header border-bottom">
                <h5 class="modal-title fw-bold text-dark" id="addDeptModalLabel">
                    <i class="bi bi-plus-circle me-2 text-primary"></i>Add New Department
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('departments.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="name" class="form-label label text-secondary">DEPARTMENT NAME</label>
                        <input type="text" name="name" id="name" class="form-control" placeholder="e.g. Finance & Billing" required>
                    </div>
                    
                    <div class="mb-3">
                        <label for="description" class="form-label label text-secondary">DESCRIPTION</label>
                        <textarea name="description" id="description" class="form-control" rows="3" placeholder="Brief details about the department..."></textarea>
                    </div>
                ...
                <div class="modal-footer border-top">
                    <button type="button" class="btn btn-ghost" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-primary">
                        Create Department <i class="bi bi-check-lg ms-1"></i>
                    </button>
                </div>
            </form>
        </div>
